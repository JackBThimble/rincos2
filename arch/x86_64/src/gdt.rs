...
pub unsafe fn init_gdt_and_tss(rsp0_top: u64) {
    unsafe {
        ...
        let tss_addr = &tss::tss_ptr() as *const _ as u64;
        TSS_DESC = make_tss_desc(tss_addr, size_of::<tss::Tss64>() as u32);

        // Build a contiguous table in memory: GDT (3 entries) + TSS desc (2 entries)
        #[repr(C, packed)]
        struct FullTable {
            gdt: [GdtEntry; 3],
            tss: TssDesc,
        ...
        let gdtr = Gdtr {
            limit: (size_of::<FullTable>() - 1) as u16,
        ...
        // Load TSS (selector points to the TSS descriptor in FULL after 3 entries => 0x18)
